...
- [新功能] 个股技术分析新增 K 线形态识别（吞没/十字星/锤子线/早晨之星/黄昏之星等）、布林带、ATR(14)、KDJ、OBV 五类扩展指标，预计算后注入 LLM Prompt 以减少幻觉、提升点位精度
- [新功能] 个股分析 Prompt 显式注入 MACD/RSI/支撑压力位数值、近 5 日 K 线走势摘要、历史信号变化趋势和板块相对强度，增强 LLM 对时间连续性和市场环境上下文的感知
- [新功能] 新增止损位合理性校验（止损价不得高于当前价）和评分-建议一致性校验，作为 LLM 输出的后处理兜底
