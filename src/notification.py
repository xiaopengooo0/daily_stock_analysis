...
class NotificationService(
    ...
    def generate_single_stock_report(self, result: AnalysisResult) -> str:
        ...
        if ext_data:
            ...
            if kline_pattern and kline_pattern not in ('N/A', '', '无明确形态'):
                kline_signal = ext_data.get('kline_signal', '')
                ext_parts.append(f"K线: {kline_pattern}" + (f"({kline_signal})" if kline_signal else ""))
            boll_upper = ext_data.get('boll_upper')
            if boll_upper and boll_upper not in ('N/A', ''):
                ext_parts.append(f"BOLL: {boll_upper}/{ext_data.get('boll_middle', 'N/A')}/{ext_data.get('boll_lower', 'N/A')}")
            atr_val = ext_data.get('atr_14')
            if atr_val and atr_val not in ('N/A', ''):
                ext_parts.append(f"ATR: {atr_val}")
            kdj_k = ext_data.get('kdj_k')
            if kdj_k and kdj_k not in ('N/A', ''):
                kdj_signal = ext_data.get('kdj_signal', '')
                ext_parts.append(f"KDJ: {kdj_k}/{ext_data.get('kdj_d', 'N/A')}/{ext_data.get('kdj_j', 'N/A')}" + (f"({kdj_signal})" if kdj_signal else ""))
            obv_trend = ext_data.get('obv_trend')
            if obv_trend and obv_trend not in ('N/A', ''):
                ext_parts.append(f"OBV: {obv_trend}")
            if ext_parts:
                lines.extend([
                    f"📐 {'Ext. Indicators' if report_language == 'en' else '扩展指标'}: {' | '.join(ext_parts)}",
                    "",
                ])
